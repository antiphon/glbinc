...
double crossprod_std(NumericMatrix &x, NumericVector& y, int l) {
  double val = 0;
  for (int i=0; i< y.size(); i++) val += x(i,l) * y(i);
  return(val);
}

void gd_binomial_std(NumericMatrix& beta,
                     NumericMatrix &X,
                     NumericVector& r,
                     NumericVector& eta,
                     int g,
  ...
  for(j=G1(g); j < G1(g+1); j++) z(j-G1(g)) = crossprod_std(X, r, j)/nd + betaold(j);
  z_norm = norm(z);
...
List glbin_lcd_std_cpp(NumericMatrix X,
                       NumericVector y,
      ...
      while(it < maxiter) {
        ...
        for(g = 0; g < J; g++) {
          if(e(g)!=0) {
            ...
            //  if(g==2) Rprintf(": %f ", beta(G1(g),l));
            gd_binomial_std(beta, X, r, eta, g, G1, l, l1, l2, df, betaold, penalty, pen_tuning);
            //  if(g==2) Rprintf("%f\n", beta(G1(g),l));
          }
        }
